...
export function CompanySection() {
  const stats = [
    { label: "Visibilidad de Cargas", value: "100%", icon: Target, color: "text-foreground", bg: "bg-white/10" },
    { label: "Disponibilidad", value: "24/7", icon: Users, color: "text-muted-foreground", bg: "bg-primary/10" },
    { label: "Cobertura de Regiones", value: "16", icon: MapPin, color: "text-muted-foreground", bg: "bg-primary/10" },
    { label: "Ahorro de Tiempo", value: "30%", icon: Rocket, color: "text-foreground", bg: "bg-primary/20" }
  ];
          ...
          >
            ...
            <div className="flex items-center gap-4">
              <div className="flex -space-x-4">
                {[1,2,3,4].map((i) => (
                  <div key={i} className="w-12 h-12 rounded-full border-2 border-background bg-muted flex items-center justify-center overflow-hidden relative">
                    {/* Using div placeholders instead of images to avoid 404s/external image issues */}
                    <div className="absolute inset-0 bg-gradient-to-br from-muted to-accent flex items-center justify-center text-xs font-bold text-muted-foreground">
                      V{i}
                    </div>
                  </div>
                ))}
              ...
              <div className="text-sm text-muted-foreground">
                <strong className="text-foreground">Un equipo 100% local</strong><br/>
