...
export default function CustomerLayout({ children }: { children: ReactNode }) {
  ...
  const isCustomer = userProfile && (userProfile as any).role === "customer";
  ...
  }, [isChecking, user, isCustomer, userProfile, router]);

  if (isChecking || !isCustomer) {
    return (
  ...
  return (
    <div className={cn("grid min-h-screen w-full transition-[grid-template-columns] duration-300 ease-in-out", isSidebarCollapsed ? "md:grid-cols-[80px_1fr]" : "md:grid-cols-[220px_1fr] lg:grid-cols-[280px_1fr]")}>
      <Sidebar role="customer" isCollapsed={isSidebarCollapsed} setCollapsed={setIsSidebarCollapsed} />
