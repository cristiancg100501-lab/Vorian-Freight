...
import { Truck, Snowflake, Layers, Calendar as CalendarIcon, MapPin, ArrowRight, ArrowLeft, Upload, Package, Ruler, Weight, Check, Star, Building, User, HelpCircle, Bolt } from 'lucide-react';
...
export default function NewShipmentPage() {
    ...
    useEffect(() => {
        const calculatePrice = async () => {
            if (!firestore || !routeDetails.distance || !routeDetails.duration || !vehicleType || !globalSettings) {
                setEstimatedPrice(0);
                setCarrierPayment(0);
                setPlatformFee(0);
                return;
        ...
        calculatePrice();
    }, [firestore, routeDetails, vehicleType, globalSettings]);
    ...
    const handleBookShipment = async () => {
        if (!firestore || !user) {
            setError("Error: No has iniciado sesión o la base de datos no está disponible.");
            return;
        }
        if (!pickup.coords || !delivery.coords) {
        ...
        try {
          await setDoc(newShipmentRef, {
            ...
            clientId: user.uid, // In admin, this could be a selectable client
            // Step 1 Data
            equipment: equipment,
            ...
            bookingMethod,
            // Metadata
            estimated_price: bookingMethod === 'instant' ? estimatedPrice : 0,
            status: 'Pending',
          ...
          console.error(err);
          setError("No se pudo crear el envío. Verifica los permisos e inténtalo de nuevo.");
        } finally {
          setIsLoading(false);
        }
    };
                      ...
                      <div className="space-y-10">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-0 mt-2 rounded-md overflow-hidden border">
                                ...
                                <EquipmentButton icon={<Snowflake className="w-5 h-5" />} label="Refrigerado" selected={equipment === 'Reefer'} onClick={() => setEquipment('Reefer')} />
                          ...
                          <div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-4">
                              <div className="flex items-center space-x-2">
                                <Checkbox id="tempControlled" checked={specialHandling.tempControlled} onCheckedChange={(checked) => setSpecialHandling(s => ({...s, tempControlled: !!checked}))} />
                                <Label htmlFor="tempControlled">Con Temperatura Controlada</Label>
                              </div>
                              <div className="flex items-center space-x-2">
