-- 🛡️ MOTOR DE PRECIOS VORIAN 4.1 (OPTIMIZADO PARA CAT 2)
-- Actualización para alinear la lógica de peajes con el estándar de categorías CAT1, CAT2, CAT3
...
BEGIN
    ...
    -- 4. POLINOMIO DE COSTO
    IF v_diesel_cost > 900 THEN v_faf_percent := ((v_diesel_cost - 900) / 20) * 0.005; END IF;
    ...
    -- 6. COMISIÓN VORIAN (15%)
    v_commission := v_subtotal_polinomio * 0.15;
    ...
    RETURN jsonb_build_object(
        ...
        'factors', jsonb_build_object(
            'diesel', v_diesel_cost,
            'vehicle', v_vehicle_type,
            'mode', v_service_mode
        )
    );
END;
