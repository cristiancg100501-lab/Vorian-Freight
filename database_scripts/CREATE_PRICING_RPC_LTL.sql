-- 🛡️ VORIAN PRICING ENGINE v6.0 - LTL (Less Than Truckload)
-- Función optimizada para cobro de carga consolidada mediante volumetría.

CREATE OR REPLACE FUNCTION public.get_vorian_price_ltl(params jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    -- 4. COSTO OPERATIVO BASE (DEL CAMIÓN COMPLETO)
    ...
    v_subtotal := v_costo_base_operativo * v_ltl_factor;
    ...
    RETURN jsonb_build_object(
        ...
        'tolls_cost', ROUND(v_standard_tolls_cost * v_ltl_factor), -- Los peajes también se ratean en el backend
        'currency', 'CLP',
        'metadata', jsonb_build_object(
            'engine', 'v6.0-LTL',
            'category', v_category_key,
            'diesel_price', v_diesel_cost,
            'volume_m3', ROUND(v_ltl_volume_m3, 2),
            'truck_fraction', ROUND(v_ltl_truck_fraction, 2)
        ),
        'factors', jsonb_build_object(
            'base_cost_diesel', ROUND(v_costo_diesel * v_ltl_factor),
            'base_cost_driver', ROUND(v_costo_tiempo * v_ltl_factor),
            'base_cost_maintenance', ROUND(v_costo_desgaste * v_ltl_factor),
            'base_margin', ROUND(v_costo_fijo * v_ltl_factor + v_commission),
            'terrain_factor', 1.0,
            'weight_factor', 1.0,
